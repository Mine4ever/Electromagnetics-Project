...
TEaLs11p=10.^(s11_m/20);    % 将分贝转为数值
TEaLs21p=10.^(s21_m/20);    % 将分贝转为数值
...
TEaRs11a=TEas11a*pi/180;    % 将角度转为弧度
TEaRs21a=TEas21a*pi/180;    % 将角度转为弧度
...
TEas11=TEaLs11p.*(cos(TEaRs11a)-1i*sin(TEaRs11a));  % 拼合相、频，组成时域表达式
TEas21=TEaLs21p.*(cos(TEaRs21a)-1i*sin(TEaRs21a));  % 拼合相、频，组成时域表达式

%%%%%%%%%%%%%%%%%%%%%%%%%%
Cv=3*10^8;
% wa = 0.0225; % WR-90
wa = 0.047;
kx=pi/wa;   % kc = sqrt((m*pi/a)^2 + (n*pi/b)^2)
k0=2*pi*fre/Cv;
ka=sqrt(k0.^2-kx^2);    % beta相位常数 = sqrt(k^2 - kc^2)
% da = 0.0097;%thick of the slab under test  被测量样品厚度
da = 0.0156;
d1a=0; %length between slab with detector of S11 靠近port1的距离
% t14 = 0.0097;%1/4 wavelength waveguide thickness   
t14 = 0.0196;
dta=t14-da; %length between slab with detector of S21   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ra=TEas11.*exp(-2i*d1a*ka);                         % 反射系数加指数项

ta=TEas21.*exp(-1i*(d1a+dta)*ka);                   % 透射系数加指数项
...
ephi1a=abs(ta./(1-ra.*(z1a-1)./(z1a+1)))<=1;        % logical number，非0即1
ephi2a=abs(ta./(1-ra.*(z2a-1)./(z2a+1)))<=1;        % logical number，非0即1
...
nnnna = na1p;%choose phase nma, na1p, na1n
